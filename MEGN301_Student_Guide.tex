...
This guide tells you what to watch out for. The \textit{MEGN 301 Master Reference Document} (Compiled Course References and Notes) tells you \textit{how things work} in depth. It is organized in three parts: Part~I follows the design-build lifecycle (Chapters~1--6: problem definition through operations), Part~II provides technical reference material for electromechanical subsystems (connectors, power supplies, motors, fasteners, power transmission, injection molding), and Part~III provides detailed milestone guides for the three formal design reviews (Chapters~13--15: PDR, CDR, FDR). The sprint-to-chapter mapping in the front matter of that document shows you exactly which chapters to consult for each sprint. Use both documents together: this one for the quick-hit tips and warnings, and the reference document for the deeper ``why'' and ``how.''
...
\end{enumerate}
If you keep these three questions in mind from the start, you will always know what your deliverables are trying to demonstrate. The Master Reference Document, Part~III (Chapters~13--15), provides detailed deliverable checklists, evaluation criteria, and common failure patterns for each review.
\end{protip}

\begin{tipbox}
\textbf{Use the Artifact Progression Table as a semester-long checklist.} Part~III of the Master Reference Document includes a master table showing how every document (SDRD, RTM, FMEA, BOM, VCRM, CAD, wiring diagrams, etc.) evolves from PDR through CDR to FDR. Print it and bring it to every design review. There is no ``starting over'' between reviews---you are maturing the \textit{same} set of documents across the semester.
\end{tipbox}
...
\begin{tipbox}
\textbf{Not sure what an acronym means?} The Master Reference Document includes a comprehensive Glossary of Acronyms and Key Terms in the back matter. If you encounter terms like VCRM, TAID, MoSCoW, RPN, ICD, or ConOps and are not sure what they mean, look them up there.
\end{tipbox}
...
Apply the \textbf{PDR Litmus Test} from Chapter~13 of the Master Reference Document: ``If a different engineering team read only our PDR package, could they independently arrive at the same concept selection and understand why it is the best choice?'' If the answer is yes, your PDR is defensible. If the answer requires ``well, they would also need to know that\ldots''---that missing information must be added to the package. Remember, PDR answers the question ``\textit{Should} this design work?'' Your deliverables must demonstrate that the problem is well-defined, the requirements are sound, and the selected concept is feasible.
\end{faqbox}
...
\begin{protip}
\textbf{Build a power budget spreadsheet before anything else.} List every component with its voltage rail, continuous current draw, and peak current draw. Sum the columns and add a 25\% margin. This single spreadsheet will tell you whether your power supply is adequate, what regulator stages you need, and where your thermal hotspots will be. Motor inrush is the number-one power supply problem: a motor rated at 2\,A running may draw 15\,A at startup. Size your supply for inrush, not steady state. See the Master Reference Document, the Power Architecture chapter (Chapter~9), for design rules and the derating guideline: 20\% margin on power supplies, 25--50\% on motors, 25\% on pumps.
\end{protip}

\begin{warnbox}
\textbf{Separate your power and signal grounds.} Poor grounding is the most common cause of hard-to-diagnose intermittent failures in student projects---noisy sensor readings, MCU resets when a motor starts, dropped serial communication. Run motor power on dedicated ground wires back to the supply; never share ground paths between motors and sensitive electronics. Use star-point grounding: each subsystem gets its own ground conductor, and they all meet at one common point near the power supply. See the Master Reference Document, the Power Architecture chapter (Chapter~9), for the three-ground-domain strategy (signal, power, safety earth).
\end{warnbox}
...
\begin{itemize}[leftmargin=*, itemsep=4pt]
    ...
    \item \textbf{Use the Master Reference Document.} The \textit{Compiled Course References and Notes} is a comprehensive technical reference organized in three parts: Part~I covers the V-model design-build lifecycle with worked GreenShield examples, Part~II provides detailed electromechanical design guidance, and Part~III provides milestone-by-milestone checklists for PDR, CDR, and FDR with evaluation criteria and common failure patterns. Consult it whenever you need deeper guidance on requirements writing, trade studies, FMEA, test planning, integration debugging, DFM, or preparing for a design review.
\end{itemize}
