...
\begin{warnbox}
\textbf{The Spring Break Shipping Trap.} Spring Break begins \springbreakstart. Campus shipping/receiving, the machine shop, and all lab facilities are \textbf{closed} that week. If a component fails during your Sprint~4 subsystem demo (around \sprintfourdemos) and you need a replacement for the full system demo, you must order it \textit{immediately}, not after break. Plan your procurement so all critical components arrive before \springbreakorderby. The same logic applies to 3D prints and machined parts: submit them early enough to allow for at least one iteration before campus shuts down.
\end{warnbox}
