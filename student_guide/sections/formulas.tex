...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=0pt]
    ...
    \item \textbf{Torque margin}: select a motor whose continuous rated torque is $\geq$1.5$\times$ the calculated load torque
...
\begin{tikzpicture}[
    ...
    tsbox/.style={rectangle, rounded corners=3pt, draw=minesblue, fill=minesblue!6,
        ...
        font=\footnotesize\sffamily\bfseries, line width=0.7pt},
    toollbl/.style={font=\scriptsize\sffamily\itshape, color=tipgreen},
    qdec/.style={diamond, draw=warnred, fill=warnred!6, aspect=2.2,
        text width=1.8cm, align=center, font=\scriptsize\sffamily\bfseries,
        inner sep=1pt, line width=0.6pt},
    arr/.style={->, line width=0.8pt, color=minesblue},
    yeslbl/.style={font=\scriptsize\sffamily, color=tipgreen},
    nolbl/.style={font=\scriptsize\sffamily, color=warnred}
]
% Title
\node[font=\normalsize\sffamily\bfseries,minesblue] at (5,5.5) {Troubleshooting First Response};

% Row 1
\node[tsbox] (s1) at (0,4) {Power OK?\\{\scriptsize All rails correct?}};
\node[toollbl,below=1pt of s1] {Multimeter};
\node[qdec] (d1) at (3.2,4) {Yes?};

% Row 2
\node[tsbox] (s2) at (6.0,4) {Signals present?\\{\scriptsize PWM, I2C, UART}};
\node[toollbl,below=1pt of s2] {Oscilloscope};
\node[qdec] (d2) at (9.2,4) {Yes?};

% Row 3
\node[tsbox] (s3) at (6.0,1.5) {Firmware running?\\{\scriptsize Serial output?}};
\node[toollbl,below=1pt of s3] {Serial monitor};
\node[qdec] (d3) at (9.2,1.5) {Yes?};

% Fix boxes
\node[tsbox,draw=warnred,fill=warnred!6] (f1) at (0,1.5) {Fix power first\\{\scriptsize Fuse, wiring,\\polarity, supply}};
\node[tsbox,draw=warnred,fill=warnred!6] (f2) at (12.2,4) {Fix signal path\\{\scriptsize Pin assignment,\\driver, wiring}};
\node[tsbox,draw=warnred,fill=warnred!6] (f3) at (12.2,1.5) {Fix firmware\\{\scriptsize Pin config, init,\\timing, state}};

% OK box
\node[tsbox,draw=tipgreen,fill=tipgreen!8] (ok) at (6.0,-0.8) {Interface fault\\{\scriptsize Isolate: disconnect\\subsystems one by one}};
