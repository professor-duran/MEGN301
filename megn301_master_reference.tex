...
% Pre-defined TikZ styles (in preamble so # parameters work inside figure environments)
\tikzset{
    slabel/.style={font=\scriptsize\sffamily\bfseries, color=#1,
        inner sep=2pt, fill=white, rounded corners=1pt},
    flow/.style={->, line width=1.2pt, color=#1, shorten <=1pt, shorten >=1pt},
    dim/.style={circle, draw=#1, fill=#1!8, minimum size=3.0cm,
        font=\scriptsize\sffamily\bfseries, align=center, line width=1pt, opacity=0.85},
    dlabel/.style={font=\scriptsize\sffamily, color=#1, align=center},
    milestone/.style={rectangle, rounded corners=3pt, draw=#1, fill=#1!8,
        text width=3.6cm, minimum height=2.2cm, align=left,
    ...
    phase/.style={rectangle, rounded corners=3pt, draw=#1, fill=#1!8,
        text width=2.4cm, minimum height=1.0cm, align=center,
        font=\scriptsize\sffamily\bfseries, line width=0.8pt},
    tier/.style={rectangle, rounded corners=2pt, draw=#1, fill=#1!12,
        minimum height=0.65cm, align=center, font=\scriptsize\sffamily\bfseries,
        line width=0.7pt},
    block/.style={rectangle, rounded corners=2pt, draw=#1, fill=#1!8,
        minimum width=2.0cm, minimum height=0.8cm, align=center,
        font=\tiny\sffamily\bfseries, line width=0.7pt},
    cat/.style={rectangle, rounded corners=3pt, draw=#1, fill=#1!10,
        text width=2.2cm, minimum height=0.8cm, align=center,
        font=\tiny\sffamily\bfseries, line width=0.7pt},
    review/.style={rectangle, rounded corners=4pt, draw=#1, fill=#1!10,
        text width=3.2cm, minimum height=2.0cm, align=center,
        font=\small\sffamily\bfseries, line width=1.0pt},
...
% Axes
\draw[->,line width=1pt,MinesBlue] (0,0) -- (12.5,0) node[below,font=\small\sffamily]{Project Phase};
...
\node[font=\scriptsize\sffamily\bfseries,AccentTeal,right] at (9.1,3.7) {ESP32};
...
\node[font=\scriptsize\sffamily,MinesSilver,right] at (9.1,3.9) {ATmega};
...
\node[font=\tiny\sffamily,diagRed,above right,inner sep=1pt] at (6.2,4.15) {Crossover at $\sim$0.35};
...
\textbf{Worked Example. GreenShield Architecture}: Grouping functions into subsystems yields: Sensor subsystem (DS18B20 + soil probe), Controller subsystem (ESP32), Actuator subsystem (relay\index{relay} + heater), Communications subsystem (SIM800L GSM module), and Power subsystem (12\,V adapter + 5\,V buck + 3.3\,V LDO\index{LDO (Low Dropout Regulator)}).
\end{greenshieldbox}
...
\textbf{Data}: protocol (I2C\index{I2C}, SPI, UART\index{UART}, WiFi), data format, message rate, error handling.
...
Rate surviving concepts 1--5 against weighted criteria (weights sum to 1.0). Calculate weighted score per option. Check sensitivity (see Figure~\ref{fig:sensitivity} for a graphical example): vary each weight $\pm$0.05; if the winner changes, the selection is fragile.
...
At minimum, analyze the top 10 failure modes covering: power system failures (short circuit, reverse polarity\index{reverse polarity protection}, battery overdischarge), sensor failures (open circuit, drift, saturation), actuator failures (stall, runaway, mechanical failure), communication failures (WiFi dropout, I2C bus hang), and software failures (watchdog timeout, state machine\index{state machine} deadlock).
...
\node[milestone=vbuild] (cdr) at (5.5,0) {
    \textbf{7 requirements}\\[2pt]
    Methods: confirmed\\
    Procedures: reviewed\\
...
\node[milestone=vverify] (fdr) at (11,0) {
    \textbf{7 requirements}\\[2pt]
    Methods: executed\\
...
\draw[arr] (pdr.east) -- node[above,font=\tiny\sffamily,MinesSilver]{Build \& test subsystems} (cdr.west);
\draw[arr] (cdr.east) -- node[above,font=\tiny\sffamily,MinesSilver]{System V\&V campaign} (fdr.west);
\end{tikzpicture}
...
\draw[gnd] (mot.south) -- ++(0,-0.5) -| node[below,font=\tiny\sffamily,diagGreen,pos=0.15]{Power GND} (0,-2.0);
\draw[gnd] (sen.south) -- ++(0,-0.7) -| node[below,font=\tiny\sffamily,diagGreen,pos=0.3]{Signal GND} (0,-2.0);
\draw[gnd] (mcu.south) -- ++(0,-0.9) -| (0,-2.0);
\fill[diagGreen] (0,-2.0) circle (2pt);
\node[font=\tiny\sffamily\bfseries,diagGreen,below] at (0,-2.1) {Star Point};
...
Varistor (MOV) & 14D471K (300\,V) & AC mains transient suppression \\
...
\node[font=\tiny\sffamily\bfseries,above right] at (9.6,2.6) {$V_{\text{out}}'<V_{\text{out}}$};
% Ground
...
\textbf{Piezoelectric}: mechanical stress generates charge (Seebeck effect in reverse). Examples: dynamic force/pressure sensors (quartz, PZT), accelerometers, ultrasonic transducers. Critical limitation: piezoelectric sensors measure only \emph{dynamic} quantities; charge leaks for static loads.
...
Force/Weight & HX711 + load cell & I2C-like & 0--200\,kg & 24-bit ADC built-in \\
Accel/Gyro & MPU-6050 & I2C & $\pm$16\,g / $\pm$2000\,\degC{}/s & 6-axis IMU \\
...
\node[dbox] at (2.0,1.3) {$\square$ Power budget (actual data, 25\% margin)};
