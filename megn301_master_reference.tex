...
\begin{itemize}[nosep,leftmargin=1.5em]
\item \textbf{Just started your project?} $\to$ Chapter~1 (Problem Definition), Chapter~3 (Requirements)
\item \textbf{Preparing for PDR?} $\to$ Chapter~4 (Concept Generation), Chapter~19 (PDR Deliverables)
\item \textbf{Need circuit basics?} $\to$ Chapter~13 (Electronics Fundamentals, Safety \& Component Selection)
\item \textbf{Designing hardware?} $\to$ Chapters~10--18 (Technical Reference for your subsystem types)
\item \textbf{Preparing for CDR?} $\to$ Chapter~20 (CDR Deliverables), verify BOM and schematics
\item \textbf{Integration is failing?} $\to$ Chapter~6 (Integration), Chapter~12 (Power Architecture)
\item \textbf{Motor won't spin?} $\to$ Chapter~11 \S11.2 (Motors \& Drivers), Chapter~12 (Power Architecture)
\item \textbf{Sensor readings are noisy?} $\to$ Chapter~13 \S13.10 (Grounding), Chapter~17 (PCB Layout Rules)
\item \textbf{Preparing for FDR?} $\to$ Chapter~5 (V\&V), Chapter~7 (O\&M), Chapter~21 (FDR Deliverables)
