...
\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{master_figs/fig_fault_isolation.png}
\caption{Fault isolation strategy\index{fault isolation}: systematic disconnection to identify the failing subsystem or interface.}\label{fig:fault_isolation}\end{figure}
...
\textbf{Step 2: Isolate.} Using the fault isolation strategy (Figure~\ref{fig:fault_isolation}), disconnect subsystems one at a time. The last one disconnected is the suspect. Or substitute a known-good signal to bypass suspect components.
