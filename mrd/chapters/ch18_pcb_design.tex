...
A 10-mil trace on 1-oz external copper safely carries only 0.3\,A. Power traces must be sized for actual current: 10\,mil\,=\,0.3\,A, 20\,mil\,=\,0.7\,A, 50\,mil\,=\,1.5\,A, 100\,mil\,=\,3.0\,A, 200\,mil\,=\,6.0\,A (per IPC-2221B\index{IPC-2221}, Section~6.2). Use the IPC-2221 calculator for exact values. Signal traces can use minimums; power traces must be sized for current.
\end{warningbox}
...
\item \textbf{Copper pour as heatsink}: for SOT-223 or D-PAK regulators, extend the ground pour under and around the package. Each square centimeter of 1-oz copper on the outer layer provides approximately 60\,\degC{}/W of thermal resistance to ambient.
...
These values assume 10\,\degC{} temperature rise above ambient, which yields higher ampacity than the IPC-2221B Section~6.2 values in the warning box earlier in this chapter (which use the IPC-2221B default temperature rise assumptions). For buried (internal) layers: derate by 50\%. For 2\,oz copper: multiply current capacity by $\sqrt{2} \approx 1.4$.
