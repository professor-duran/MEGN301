...
\input{tikz_figures/fig_pcb_layer_stackup}
...
A 10-mil trace on 1-oz external copper safely carries only 0.3\,A. These are conservative/derated values with safety margin for quick reference: 10\,mil\,=\,0.3\,A, 20\,mil\,=\,0.7\,A, 50\,mil\,=\,1.5\,A, 100\,mil\,=\,3.0\,A, 200\,mil\,=\,6.0\,A. Use the IPC-2221 calculator for exact values. Signal traces can use minimums; power traces must be sized for current. See the detailed table in Section~\ref{sec:trace_current} for IPC-2221B\index{IPC-2221} values at 10\,\degC{} rise.
\end{warningbox}
...
\begin{figure}[htbp]\centering
\input{tikz_figures/fig_pcb_design_workflow}
...
A professional schematic is a \emph{communication document}, not just a wiring diagram. Follow these rules from IPC-2612:
...
\subsection{Trace Width and Current Capacity}\label{sec:trace_current}
The PCB trace must be wide enough to carry the required current without excessive temperature rise. The following values are per IPC-2221B at 10\,\degC{} temperature rise for external layers on 1\,oz copper (35\,$\mu$m thick). These are slightly higher than the conservative/derated quick-reference values in the warning box above, which include additional safety margin:
