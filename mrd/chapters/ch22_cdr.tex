...
The Critical Design Review is the second formal design gate, following PDR. Where PDR confirmed that the team selected a feasible concept, CDR confirms that the concept has been developed into a complete, buildable design.
...
\begin{tipbox}[How CDR Differs From PDR]
CDR requires a completed, fabrication-ready design---not just a feasible concept. At PDR, the team demonstrated that the selected concept \emph{should} work (feasibility-level analysis, preliminary architecture, concept sketches). At CDR, every component is specified by part number, every interface is defined with pinouts and wire gauges, every custom part has dimensioned and toleranced drawings, and every analysis uses actual component data rather than estimates. PDR earns the right to spend money on design; CDR earns the right to build.
\end{tipbox}
