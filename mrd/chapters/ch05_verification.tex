...
Each step is a single action with an expected result. Use specific values (``Set reference bath to 25.0 $\pm$ 0.1\,\degC{}''), not vague instructions (``check temperature''). Include wait times for stabilization. Imperative voice: ``Set\ldots'' ``Record\ldots'' ``Verify\ldots''
...
\item Classify each requirement by verification method (T/A/I/D): (a) ``Shall withstand 50\,N load without permanent deformation.'' (b) ``Shall display the company logo on the front panel.'' (c) ``Shall complete a power-on sequence within 10 seconds.'' (d) ``Shall have a thermal margin of $\geq$20\,\degC{} at max power under worst-case ambient.''
\item Write a complete test procedure (all 10 sections) for verifying: ``SYS-REQ-007: The system shall measure ambient temperature with accuracy of $\pm$0.5\,\degC{} over 0 to 50\,\degC{}.''
\item A test result shows 0.6\,\degC{} error against a requirement of $\pm$0.5\,\degC{}. What is the verdict? Describe the three possible root causes and the investigation sequence.
...
\item Purpose: verifies SYS-REQ-007. Setup: NIST-traceable temperature bath ($\pm$0.05\,\degC{}), calibrated reference thermometer, DUT powered and warmed 30 min. Preconditions: DUT firmware v2.1, ambient 23$\pm$3\,\degC{}. Steps: (1) Set bath to 0.0\,\degC{}, wait 10 min, record DUT and reference. (2) Repeat at 10, 20, 30, 40, 50\,\degC{}. Pass/fail: $|$DUT $-$ reference$|$ $\leq$ 0.5\,\degC{} at every point. Data: table with columns for setpoint, reference, DUT, error. Safety: bath uses water, standard lab PPE. Signatures: tester, witness, date.
\item Verdict: FAIL (0.6 $>$ 0.5). Investigation: (1) Check test: was the reference bath stable? Was the reference thermometer calibrated? Repeat with verified equipment. (2) Check implementation: sensor calibration coefficients, ADC linearity, thermal coupling. (3) Check requirement: was $\pm$0.5\,\degC{} justified? If the application can tolerate $\pm$1.0\,\degC{}, a requirements change (with stakeholder approval) may be appropriate. Open a DR regardless.
