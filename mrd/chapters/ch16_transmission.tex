...
\textbf{Spur}: straight teeth, 94--98\%/mesh, noisy at speed. \textbf{Helical}: angled teeth, quieter, generates axial thrust. \textbf{Bevel}: intersecting shafts (\ang{90}). \textbf{Worm}: high ratio single-stage (5:1--100:1), 40--90\% efficient, \textbf{self-locking} when lead angle $<$$\sim$\ang{6}. \textbf{Planetary}: compact, coaxial, 95--97\%/stage.
...
\textbf{Lead screws}: a threaded rod turns in a nut, translating rotary motion to linear motion. The lead $l$ (linear distance per revolution) determines the speed and force: $v = l \cdot n$ (speed), $F = 2\pi T \eta / l$ (force, motor driving load). Standard trapezoidal (ACME) thread: $\eta \approx 40\%$. Ball screws (recirculating ball bearings in the nut): $\eta \approx 90\%$, but 10$\times$ the cost.
...
\textbf{Selection criteria}: (1) torque rating $\geq 2\times$ the maximum torque (safety factor for shock loads), (2) bore size matches both shafts, (3) misalignment tolerance $\geq$ the expected misalignment (typically $\pm$\ang{1} angular and $\pm$0.5\,mm parallel for student assemblies), (4) set screw or clamp style (clamp is preferred; set screws can loosen and damage the shaft).
...
\textbf{Lead screw actuator}: motor $\to$ coupling $\to$ lead screw $\to$ nut. Linear speed $v = l \times n$ (lead $\times$ RPM). Force $F = 2\pi T_{\text{motor}} \times i / l \times \eta$. Self-locking when the lead angle $< \arctan(\mu)$ (typically $<$\ang{5}), meaning the load cannot back-drive the motor. This is useful for holding position without power.
...
6001 bearing (12\,mm ID) & ABEC-1 & McMaster & Idler pulleys, larger shafts \\
