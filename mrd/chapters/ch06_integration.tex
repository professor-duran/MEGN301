...
\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{master_figs/fig_integration_plan.png}
\caption{Integration sequence plan\index{integration planning}: power-first, risk-ordered subsystem build-up with test gates.}\label{fig:integration_plan}\end{figure}

The order of integration matters (Figure~\ref{fig:integration_plan}). Rules: (1) Integrate power first; nothing works without power. (2) Add the lowest-risk subsystem next. (3) Add the highest-risk subsystem last (so problems are isolated to one new element). (4) Never add two new elements simultaneously (if something breaks, you cannot isolate the cause).
...
\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{master_figs/fig_interface_types.png}
\caption{The six interface types\index{interface types}: electrical, mechanical, thermal, software, fluid, and human.}\label{fig:interface_types}\end{figure}

80\% of integration failures occur at interfaces (Figure~\ref{fig:interface_types}). Six types: electrical (voltage, protocols, grounding), mechanical (mounting, tolerances, alignment), thermal (heat flow, contact resistance), software (APIs, data formats, timing), fluid (fittings, pressure, seals), and human (displays, controls, ergonomics).
...
\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{master_figs/fig_handoff_package.png}
\caption{Integration handoff package\index{handoff package}: documentation artifacts that transfer from design to integration.}\label{fig:handoff_package}\end{figure}

The integration handoff package (Figure~\ref{fig:handoff_package}) ensures documentation transfers cleanly from design to build.
